...
=====================================================
Bilingual by design – English default, Italian optional
-------------------------------------------------------
* **openai‑python ≥ 1.0** interface (`OpenAI()`)
* `--auth` flag → prints refresh‑token once
* `--lang en|it` to pick script + TTS language
...
# daily short in English (default)
python faceless_short_automation.py

# local short in Italian, no upload
python faceless_short_automation.py --lang it --no-upload
```
In GitHub Actions puoi impostare la variabile d’ambiente `LANGUAGE=en` o `it` al posto di `--lang`.
"""
...
WORKDIR        = Path(tempfile.gettempdir()) / "short_builder"; WORKDIR.mkdir(exist_ok=True)
TARGET_DURATION = 18  # seconds
FONT           = "Montserrat-Bold"
DEFAULT_LANG   = os.getenv("LANGUAGE", "en")
VOICE_ID = {
    "en": "EXAVITQu4vr4xnSDxMaL",  # English voice
    "it": "TxGEqnHWrfWFTf9VQmLc",  # Italian voice
}
...
def generate_script(topic: str, lang: str) -> str:
    if lang == "it":
        prompt = f"Scrivi un copione divertente in 3 fatti su {topic} in massimo 60 parole. Termina con una domanda per incoraggiare i commenti."
    else:
        prompt = f"Write a fun, 3‑fact script about {topic} in ≤60 words. End with a question to encourage comments."
...
def fetch_vertical_clip(query: str) -> Path:
    ...
    with requests.get(file_link, stream=True, timeout=60) as src, open(out, "wb") as dst:
        for chunk in src.iter_content(8192): dst.write(chunk)
    return out

# ──────────────────────────────────────────────────────────
# 3 ▸ AI voice‑over (ElevenLabs)
# ──────────────────────────────────────────────────────────
...
def generate_voiceover(text: str, lang: str) -> Path:
    ...
    r = requests.post(url, headers={"xi-api-key": ELEVEN_KEY, "Content-Type": "application/json"},
...
# ──────────────────────────────────────────────────────────
# 4 ▸ Assemble video + captions
# ──────────────────────────────────────────────────────────

def build_video(clips: list[Path], audio: Path, script: str, out_path: Path):
    seg = TARGET_DURATION / len(clips)
    vid = concatenate_videoclips([VideoFileClip(str(p)).subclip(0, seg) for p in clips], method="compose")
    vid = vid.set_audio(AudioFileClip(str(audio)))
    caption = TextClip(textwrap.fill(script, 30), fontsize=60, font=FONT, color="white",
                       stroke_color="black", stroke_width=2, size=(vid.w*0.9, None), method="caption")
...
def run_once(lang: str, upload: bool):
    ...
    if upload:
        ...
        upload_short(out_file, title, script)
